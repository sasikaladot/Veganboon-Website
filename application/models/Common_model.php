...
public function get_login_detail($username,$password)
{
	$this->db->select('*');
		...
		$this->db->where(array('admin_password'=>md5($password),'admin_status'=>1));
		$query = $this->db->get();
		return $query;
...
public function affiliatePointUpdate($tablename,$where,$affiliate_point){
    $this->db->where($where);
    $this->db->set('user_affiliate_points', 'user_affiliate_points + ' . $affiliate_point, FALSE);
    $this->db->update($tablename);
}


public function affiliatePointReduce($tablename,$where,$affliateamount){
    $this->db->where($where);
    $this->db->set('user_affiliate_points', 'user_affiliate_points - ' . $affliateamount, FALSE);
    $this->db->update($tablename);
}
